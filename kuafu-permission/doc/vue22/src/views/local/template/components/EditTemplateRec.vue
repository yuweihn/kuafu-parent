<template>
	<el-dialog title="编辑模板明细" :visible.sync="editFormVisible" width="500px" :close-on-click-modal="true"
		...
		<div slot="footer" class="dialog-footer">
			...
			<el-button type="primary" @click.native="editSubmit" :loading="editLoading" v-hasPerm="['template.rec.update']">提交</el-button>
		</div>
	</el-dialog>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			show: function(templateId, index, row) {
			...
			editSubmit: function() {
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						var params = "_fk=u" + "&id=" + this.editForm.id + "&templateId=" + this.templateId
								 + (this.editForm.name ? "&name=" + this.editForm.name : "")
                                 + (this.editForm.value ? "&value=" + this.editForm.value : "")
                                 + "&orderBy=" + this.editForm.orderBy;
						this.editLoading = true;

						this.$axios.post(this.$global.baseUrl + '/template/rec/update', params).then((res) => {
