<template>
	<el-dialog title="新增反向代理目标参数模板" :visible.sync="addFormVisible" width="500px" :close-on-click-modal="true"
		...
		<div slot="footer" class="dialog-footer">
            ...
            <el-button type="primary" @click.native="addSubmit" :loading="addLoading" v-hasPerm="['server.location.param.template.create']">提交</el-button>
		</div>
	</el-dialog>
</template>
		...
		methods: {
			...
			addSubmit: function() {
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						var params = "_fk=u" + "&locationId=" + this.locationId
								 + (this.addForm.templateId ? "&templateId=" + this.addForm.templateId : "");
						this.addLoading = true;

						this.$axios.post(this.$global.baseUrl + '/server/location/param/template/create', params).then((res) => {
