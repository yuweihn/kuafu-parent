<template>
	<el-dialog title="编辑反向代理目标参数模板" :visible.sync="editFormVisible" width="500px" :close-on-click-modal="true"
    ...
    <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click.native="editSubmit" :loading="editLoading" v-hasPerm="['server.location.param.template.update']">提交</el-button>
    </div>
	</el-dialog>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			show: function(locationId, index, row) {
			...
			editSubmit: function() {
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						var params = "_fk=u" + "&id=" + this.editForm.id + "&locationId=" + this.locationId
								 + (this.editForm.templateId ? "&templateId=" + this.editForm.templateId : "")
                                 + "&orderBy=" + this.editForm.orderBy;
						this.editLoading = true;

						this.$axios.post(this.$global.baseUrl + '/server/location/param/template/update', params).then((res) => {
