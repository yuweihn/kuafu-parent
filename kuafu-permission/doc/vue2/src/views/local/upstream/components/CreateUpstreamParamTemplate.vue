<template>
	<el-dialog title="新增负载均衡参数模板" :visible.sync="addFormVisible" width="500px" :close-on-click-modal="true"
		...
		<div slot="footer" class="dialog-footer">
			...
			<el-button type="primary" @click.native="addSubmit" :loading="addLoading" v-hasPerm="['upstream.param.template.create']">提交</el-button>
		</div>
	</el-dialog>
</template>
		...
		methods: {
			...
			addSubmit: function() {
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						var params = "_fk=u" + "&upstreamId=" + this.upstreamId
								 + (this.addForm.templateId ? "&templateId=" + this.addForm.templateId : "");
						this.addLoading = true;

						this.$axios.post(this.$global.baseUrl + '/upstream/param/template/create', params).then((res) => {
