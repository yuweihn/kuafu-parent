...
public class PermissionCheckInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		for (String permNo : permNos) {
			...
			boolean has = sysAdminService.hasPermission(adminId, permissionDto.getId());
			if (has) {
				ok = true;
				break;
			}
	...
	protected void outputErrorMsg(String[] permNos) {
		...
		ActionUtil.export(content.getBytes(StandardCharsets.UTF_8), ContentType.APPLICATION_JSON.getMimeType());
	}
