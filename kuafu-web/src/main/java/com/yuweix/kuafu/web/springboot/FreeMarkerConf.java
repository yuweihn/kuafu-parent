...
import com.yuweix.kuafu.web.Page;
import com.yuweix.kuafu.web.freemarker.FreemarkerUtil;
...
public class FreeMarkerConf {
	...
	public FreeMarkerConfigurer freemarkerConfig(
			@Value("${kuafu.freemarker.template.path:classpath:/templates/}") String template_path,
			@Value("${kuafu.freemarker.template.update.delay:0}") String template_update_delay,
			@Value("${kuafu.freemarker.default.encoding:utf-8}") String default_encoding,
			@Value("${kuafu.freemarker.output.encoding:utf-8}") String output_encoding,
			@Value("${kuafu.freemarker.locale:zh_CN}") String locale,
			@Value("${kuafu.freemarker.number.format:0.##########}") String number_format,
			@Value("${kuafu.freemarker.date.format:yyyy-MM-dd}") String date_format,
			@Value("${kuafu.freemarker.time.format:HH:mm:ss}") String time_format,
			@Value("${kuafu.freemarker.datetime.format:yyyy-MM-dd HH:mm:ss}") String datetime_format,
			@Value("${kuafu.freemarker.classic.compatible:true}") String classic_compatible,
			@Value("${kuafu.freemarker.template.exception.handler:ignore}") String template_exception_handler,
			@Value("${kuafu.freemarker.whitespace.stripping:true}") String whitespace_stripping,
			@Value("${kuafu.freemarker.registered.custom.output.formats:}") String customOutputFormats,
			@Value("${kuafu.freemarker.auto.import:}") String auto_import) {
	...
	@ConditionalOnMissingBean(FreemarkerUtil.class)
	@Bean
